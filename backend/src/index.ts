...
const configuration = new OpenAI({
    openAIApiKey: "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",
    temperature: 0.1,
});
...
/*Contar vocales*/
const PrompCont = (text: string) => {
    return `Tu tienes un rol de contar y quiero que cuentes las vocales de este texto: "${text}"`;
}
app.post('/contar', async (req, res) => {
    try {
        const { prompt } = req.body;
        const completion = await openai.completionWithRetry({
            model: 'gpt-3.5-turbo-instruct',
            prompt: PrompCont(prompt),
